...
class RegressionTree2(metadata: Array[String]) extends Serializable {
  ...
  def buildTree(trainingData: RDD[String], yFeature: String = featureSet.data(yIndex).Name, xFeatures: Set[String] = Set[String]()): Node = {
    ...
    var fYindex = featureSet.data.indexWhere(p => p.Name == yFeature) // PM: pay attention if you use small RDDs!
    ...
    def buildIter(conditions: List[Condition]): Node = {
      ...
      if (stopExpand) {
        ...
        if (splittingPointFeature.index == -1) { // the chosen feature has only one value
          ...
          val chosenFeatureInfoCandidate = featureSet.data.find(f => f.index == splittingPointFeature.index)
